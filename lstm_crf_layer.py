...
class BLSTM_CRF(object):
    ...
    def __init__(self, embedded_chars, hidden_unit, cell_type, num_layers, droupout_rate,
                 ...
                 initializers, num_labels, seq_length, labels, lengths, is_training, max_seq_length):
    ...
    def add_blstm_crf_layer(self):
        ...
        # project
        logits = self.project_fullyconnected_layer(lstm_output)
        ...
        # # crf
        log_likelihood, trans = self.crf_layer(logits)
        pred_ids, _ = crf.crf_decode(potentials=logits, transition_params=trans, sequence_length=self.lengths)  # CRF decode, pred_ids 是一条最大概率的标注路径

        loss = tf.reduce_mean(-log_likelihood)

        return (loss, per_example_loss, pred_ids, probabilities)
    ...
    def project_fullyconnected_layer(self, lstm_outputs, name=None):
        ...
        with tf.variable_scope("project" if not name else name):
            with tf.variable_scope("hidden"):
                ...
                W = tf.get_variable("W", shape=[self.hidden_unit * 2, self.hidden_unit], dtype=tf.float32)
            ...
            with tf.variable_scope("logits"):
                ...
                #
                #                     dtype=tf.float32, initializer=self.initializers.xavier_initializer())
                W = tf.get_variable("W", shape=[self.hidden_unit, self.num_labels], dtype=tf.float32)
